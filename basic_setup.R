# Model a Quantitative Trading Strategy in R - DataCamp
# 
# Get familiar with R! This section is to lay the foundation of programming
# skills in R before proceeding to strategy writing. It covers reading a
# data.table, creating new columns in the table, calculating returns by
# different methods, loop-functions, conditional functions and 
# plotting of the dataset.
# ============================================================================
# 1. INTRO TO R FOR TRADING
# ============================================================================
# Download data of a stock called MARUTI from yahoo
# The data set contains Maruti's daily OHLC (Open/High/Low/Close) and
# volumn data from Jan-1-2008 to Dec-31-2013


# 1.1 Using R to work with data
# ============================================================================
#### Read a .csv file
data_maruti <- read.csv(file="http://assets.datacamp.com/course/quantinsti/data_ch1.csv", header = TRUE, stringsAsFactors = FALSE)

#### Check the typeof(), class(), dim() of your R data
# Working with data.frame which is a lot like excel tables and 
# comprised of vectors of different types of variables
typeof(data_maruti)
class(data_maruti)
dim(data_maruti)

#### Extract data from data.frame
# A single cell: df_name[x, y] | df_name$col_name[n]
# An entire row: df_name[x, ]
# An entire column: df_name[ ,y] | df_name$col_name
data_maruti$Date[1]
data_maruti$Date[nrow(data_maruti)-1]
# Consecutive cells in a column df_name$col_name[row n : row n]
data_maruti$Date[1:2]

#### Using function c() to build the data
# Create a column lastClose for the previous-day closing prices
# 1. Copy the values from the column of data_maruti$Close
# 2. Shift the values by one down in data_maruti$lastClose
# OUTCOME Example: data_maruti$Close[i] = data$maruti$lastClose[i+1]
# NOTE: data_maruti$Close[0] doesn't exist so the value 0 is assigned to data$maruti$lastClose[1]
# NOTE: data_maruti$Close[nrow(data_maruti)] is outside the range of 
# data.frame and will not be included
data_maruti$lastClose <- c(0, data_maruti$Close[1:nr-1])
# Add a column of the previous-day opening prices
data_maruti$lastOpen <- c(0, data_maruti$Open[1:nr-1])


# 1.2 Calculate Returns Using c() and diff()
# ============================================================================
# Get the difference between 2 pairs of consecutive opening prices
difference1 <- data_maruti$Close - c(0, data_maruti$Close[1:nrow(data_maruti)-1])
# NOTE: Using diff() returns 1000 as the first value which is wrong
difference <- diff(data_maruti$Close, lag = 1)
# Get the simple return for one cell
Close1 <- data_maruti$Close[1]
Close2 <- data_maruti$Close[2]
data_maruti$sreturns[2] <- data_maruti$diffClose[2]/Close1
data_maruti$sreturns[3] <- data_maruti$diffClose[3]/Close2
# 1. Get the difference between every pair of consecutive closing prices
data_maruti$diffClose <- c(0, diff(data_maruti$Close, lag = 1))
# 2. Create a new column called sreturns to store the values of returns
data_maruti$sreturns <- 0
# 3. Get the number of observations in the data.frame
nrows <- nrow(data_maruti)
# 4. Create vector denom for the the denominator of the division and assign the value 1 to be the first closing price
denom <- c(1, data_maruti$Close[1:nrows-1])
# 5. Calculate the simple returns using vector division
data_maruti$sreturns <- data_maruti$diffClose / denom
# 6. Calculate the simple returns using for-loop
# The for-loop will run from 2 to nrows i.e. the last row
data_maruti$sreturns <- 0
for( i in 2:nrows) {
  # Calculate returns for each row
  data_maruti$sreturns[i] <- data_maruti$diffClose[i] / data_maruti$Close[i-1]
}


# 1.3 Calculate Continuously Compounded Returns using log()
# ============================================================================
# Calculate CC returns using vector division method
data_maruti$ccreturns1 <- 0
data_maruti$ccreturns1 <- c(0, diff(log(data_maruti$Close), lag = 1))
data_maruti$ccreturns2 <- 0
for ( i in 2:nrows) {
  data_maruti$ccreturns2[i] <- log(data_maruti$Close[i]) - log(data_maruti$Close[i-1])
}


# 1.4 Visualize the data
# ============================================================================
# See the behaviour of the closing prices
plot(data_maruti$Close,
     type = "l",
     main = "Maruti Closing Values",
     xlab = "Date",
     ylab = "Index Value",
     col = "red")
# See the simple returns
plot(data_maruti$sreturns,
     type = "l",
     main = "Maruti Simple Returns",
     xlab = "Date",
     ylab = "Returns",
     col = "red")
# See the Continuously Compounded Returns
plot(data_maruti$ccreturns1,
     type = "l",
     main = "Maruti Continuously Compounded Returns",
     xlab = "Date",
     ylab = "Returns",
     col = "blue")


# ============================================================================
# 2. CODE A BASIC TRADING STRATEGY
# ============================================================================
# ============================================================================
# 2.1 Define the Strategy
# If Price movement from time "t1" to time "t2" is greater than or equal to 
# "parameter" trigger an order which is opposite to the existing one.
# If the existing order is "bought", place a order for sell.
# If the existing order is "sold", place a order for buy.
# ============================================================================
# ============================================================================
# 2.2 Strategy Assumptions
# Only one trading lot is allowed
# Always one trade is open after first signal
# - Trading quantity for 1st signal = 1 lot
# - Trading quantity for subsequent signals = 2 lots
# EXAMPLE: After 1st signal (buy 1 lot, always a buy), 
#          2nd signal (sell 2 lots): 1 sell cancels 1 buy, net 1 lot sold
#          3rd signal (buy 2 lots): 1 buy cancels a sell, net 1 lot buy
#          ...
# Trade price is always market order
# - When signal says "sell": Bid Price
# - When signal says "buy": Ask Price
# ============================================================================
# ============================================================================
# 2.3 Strategy Parameter and tradedata
# PARAMETER: A pre-determined absolute difference between p(t2) - p(t1)
# INPUT DATA: Prices - OHLC but only using Close
#             Bid price - Best price at which someone is willing to buy
#                       - Sell orders at this price
#             Ask price - Best price at which someone is willing to sell
#                       - Buy orders at this price
#             Time stamps - Use serial number(1,2,3,4,...) as proxy for time stamp
# OUTPUT DATA: signal
#              tqty
#              tprice
#              tprofit
#              mtm (mark to market) - Total profit from the strategy up till a row
# ============================================================================
# ============================================================================
# 2.4 Trading Signal Generation Logic
# 2.4.1 Whether to buy or sell
# 2.4.2 At what price
# 2.4.3 How much do we buy or sell
# This strategy is to look at the differences between the 
# closing prices and the existing positions to generate the buy or sell signals
# CRITERION: if p(t2)-p(t1) >= parameter then buy/sell else no trade
# CURRENT POSITION:
# If the closing price of the previous day is greater than today then 
# currpos = 0 (No position)
# currpos = 1 (Bought position)
# currpos = -1 (Sold position)
# Buy/Sell: If not bought, buy now, else sell now
# ============================================================================
# ============================================================================
# 2.5 Output Variables
# 2.5.1 During execution: 
#   - tqty: Trading quantity is the number of trading lots
#   - tprice: Trading price for the marker order
#   - tprofit: Trading profit calculated for the order placed/executed in a specific row
# 2.5.2 After execution:
#   - mtm: Add tprice from all the rows till row i + currpos in row i * closing price in row i
#   - Traded lots: Absolute sum of tqty + 1


# Load the dataset
# ============================================================================
tradedata <- read.csv("http://assets.datacamp.com/course/quantinsti/data_ch2.csv", header = TRUE, stringsAsFactors = FALSE)

# Initialize the variables for the PNL (profit and loss) table to the dataset
# ============================================================================
# signal for the particular row, buy, sell, or no change
tradedata$signal <- ""
# quantity to be traded
tradedata$tqty <- 0
# price at which a trade would take place.
# For selling, this equals the best bid, for buying this is the best ask.
tradedata$tprice <- 0
# profit for a particular trade.
tradedata$tprofit <- 0
# cumulative profit after squaring off the last open position till that point
tradedata$mtm <- 0

# Define & Initialize Counters
# ============================================================================
# total number of rows in the dataframe
nrows <- nrow(tradedata)
# current position: 0 - no position, 1 - bought position, -1 - sold position
currpos <- 0
# set the parameter
parameter <- 5

# The logic behind the if condition for signal generation
# for row 2 to nrows in data frame:
#   if absolute price difference >= parameter:
#     if condition for buying is met:
#       update the output columns to "buy"
#     else
#       update the output columns to "sell"
#     endif
#   else
#     update the output columns to "change"
#
#   update profit columns
#
#   endfor


# if condition for signal generation
# ============================================================================
for ( i in 2:nrows ) {
  # Test the whether there is a trigger based on the parameter
  if ( abs( tradedata$Price[i] - tradedata$Price[i-1] ) >= parameter ) {
    # if the diff between the prices is greater than parameter &
    # Test the state of currpos
    if ( currpos < 1 ) {
      # The currpos is not bought
      tradedata$signal[i] <- "buy"
      tradedata$tprice[i] <- tradedata$Ask[i]
      tradedata$tqty[i] <- if ( currpos == 0 ) { 1 } else { 2 }
      currpos <- currpos+tradedata$tqty[i]
    } 
    else {
      # currpos is sold
      tradedata$signal[i] <- "sell"
      tradedata$tprice[i] <- tradedata$Bid[i]
      tradedata$tqty[i] <- if ( currpos == 0 ) { -1 } else { -2 }
      currpos <- -1
    }
  }
  else {
    # no change
    tradedata$signal[i] <- "no change"
    tradedata$tprice[i] <- 0
    tradedata$tqty[i] <- 0
  }
  # tprofit
  tradedata$tprofit[i] <- - tradedata$tqty[i] * tradedata$tprice[i]
  # mtm
  tradedata$mtm[i] <- sum(tradedata$tprofit[1:i]) + currpos * tradedata$Price[i]
}


# Calculate Profit & Traded Lots
# ============================================================================
# Did my strategy got any fills? num_traded_lots
# Did my strategy make any profit? mtm_final
num_traded_lots <- sum(abs(tradedata$tqty)) + 1
mtm_final <- tradedata$mtm[nrows]



